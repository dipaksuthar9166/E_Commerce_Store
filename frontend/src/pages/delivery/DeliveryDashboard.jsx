...
import {
  MapPin, Navigation, CheckCircle, XCircle, Clock,
  TrendingUp, Bike, IndianRupee, Package, ChevronRight,
...
const ActiveDeliveryCard = ({ task, onDeliver }) => {
...
const DeliveryDashboard = () => {
  ...
  useEffect(() => {
    ...
    const handleTaskTaken = (takenId) => {
      setTasks(prev => prev.filter(t => t._id !== takenId));
    };
  ...
  const fetchRiderData = async () => {
    try {
      ...
      // If rider has an active out_for_delivery task, bind it to activeDelivery
      const activeOrder = history.find(o => o.status === 'out_for_delivery');
      if (activeOrder) {
        setActiveDelivery({
          _id: activeOrder._id,
          shop: activeOrder.shop,
          shopAddress: 'Assigned Shop Address',
          distance: '2.5 km',
  ...
  const handleDeliver = async (id, otp) => {
    try {
      ...
      fetchRiderData(); // reload stats and history
    } catch (error) {
  ...
  return (
    <div className="bg-gray-50 min-h-full">
      ...
      <div className="px-4 -mt-2 space-y-4 pb-4">
        ...
        {/* ── Active Delivery ───────────────────────── */}
        {activeDelivery && (
          <ActiveDeliveryCard task={activeDelivery} onDeliver={handleDeliver} />
        )}

        {/* ── Map Placeholder ───────────────────────── */}
        <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
          <div className="relative bg-gray-200 h-40 flex flex-col items-center justify-center gap-2">
            {/* Fake map grid lines */}
            ...
            <div className="relative z-10 flex flex-col items-center gap-2">
              <div className="w-10 h-10 rounded-full bg-blue-600/90 flex items-center justify-center shadow-lg animate-bounce">
                <Navigation className="w-5 h-5 text-white" />
              </div>
              <span className="text-gray-600 font-semibold text-xs bg-white/80 px-3 py-1 rounded-full shadow">
                Map Loading...
              </span>
            </div>
          </div>
        ...
        {tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3 text-gray-400">
            ...
            <p className="text-xs text-center text-gray-400 leading-tight">Stay online. New accepted vendor orders will flash here in real-time!</p>
          </div>
        ) : (
          tasks.map(task => (
            <TaskCard
